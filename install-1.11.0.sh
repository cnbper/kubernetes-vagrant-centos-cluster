...
# yum -y install --downloadonly --downloaddir=/vagrant/rpm/kubeadm/1.11.0 kubernetes-cni-0.6.0 kubelet-1.11.0 kubeadm-1.11.0 kubectl-1.11.0 --disableexcludes=kubernetes
yum localinstall -y /vagrant/rpm/kubeadm/1.11.0/*.rpm

cat > /etc/sysconfig/kubelet <<EOF
KUBELET_EXTRA_ARGS=" --pod-infra-container-image=registry.sloth.com/google_containers/pause-amd64:3.1"
EOF
...
then
    ...
    # kubeadm config images list --kubernetes-version=1.11.0
    # kubeadm config print-default --api-objects=MasterConfiguration
    # 配置 kubernetesVersion: v1.11.0
    ...
    # 配置 外部etcd
    kubeadm init --config=/vagrant/yaml/kubeadm-init-v1.11.0.yaml
